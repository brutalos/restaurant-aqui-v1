...
import { Calendar, Users, Clock, Send, Trophy } from 'lucide-react'

const BowlingReservationForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/reservations/bowling', {
        method: 'POST',
      ...
      console.error('Bowling submit error:', error)
      setStatus('error')
    }
  }
    ...
    return (
      <div className="bg-white p-12 md:p-20 shadow-2xl border-l-8 border-primary text-center">
        ...
        <p className="text-gray-500 mb-8">
          Ihre Reservierungsanfrage für eine Bowlingbahn ist bei uns eingegangen. Wir werden uns in Kürze zur Bestätigung bei Ihnen melden.
        </p>
        <button 
          onClick={() => {
            setStatus('idle')
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-12 md:p-20 shadow-2xl space-y-10 border-l-8 border-primary">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="space-y-4">
          <label className="text-[10px] uppercase tracking-[0.4em] font-black text-gray-400">Anzahl Bahnen</label>
          <div className="relative">
            <Trophy className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={18} />
            <select
              name="lanes"
            ...
            >
              {[1, 2, 3, 4].map(num => (
                <option key={num} value={num}>{num} {num === 1 ? 'Bahn' : 'Bahnen'}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      >
        ...
        <span className="absolute flex items-center justify-center w-full h-full text-white transition-all duration-300 transform group-hover:translate-x-full ease uppercase tracking-[0.2em]">
          {status === 'loading' ? 'Wird gesendet...' : 'Bahn Reservieren'}
        </span>
...
export default BowlingReservationForm
